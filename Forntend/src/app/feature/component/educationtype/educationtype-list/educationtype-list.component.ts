...
@Component({
  selector: 'app-educationtype-list',
  templateUrl: './educationtype-list.component.html',
  styleUrls: ['./educationtype-list.component.scss']
})
export class EducationtypeListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<EducationType> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public educationtypeService: EducationtypeService, public routeService: RouteService
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.educationtypeService.delete(projectId).subscribe((e) => {
