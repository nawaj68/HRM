...
export class LanguageListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["languageName", "employeeId", "proficencyId", "remark", "action"];
  ...
  load(): any {
    return this.languageService.getSearch(
  ...
  remove(languageId: number): void {
    if (languageId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.languageService.delete(languageId).subscribe((e) => {
