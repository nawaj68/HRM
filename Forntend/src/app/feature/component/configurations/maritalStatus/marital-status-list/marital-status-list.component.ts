...
export class MaritalStatusListComponent implements OnInit {
  ...
  customSubscribe = (data: any) => {
    this.maritalStatus = data.data.data;
  ...
  load(): any {
    return this.maritalStatusService.getSearch(
  ...
  remove(maritalStatusId: number): void {
    if (maritalStatusId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.maritalStatusService.delete(maritalStatusId).subscribe((e) => {
