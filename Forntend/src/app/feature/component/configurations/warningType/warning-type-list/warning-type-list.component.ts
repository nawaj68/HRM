...
@Component({
  selector: 'app-warning-type-list',
  templateUrl: './warning-type-list.component.html',
  styleUrls: ['./warning-type-list.component.scss']
})
export class WarningTypeListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<WarningType> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public warningTypeService: WarningTypeService, public routeService: RouteService
    ...
    load(): any {
      return this.warningTypeService.getSearch(
    ...
    remove(warningTypeId: number): void {
      if (warningTypeId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.warningTypeService.delete(warningTypeId).subscribe((e) => {
