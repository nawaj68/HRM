...
export class ProficiencyListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<Proficiency> = new MatTableDataSource();
  ...
  customSubscribe = (data: any) => {
    this.proficiency = data.data.data;
  ...
  load(): any {
    return this.proficiencyService.getSearch(
  ...
  remove(proficiencyId: number): void {
    if (proficiencyId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.proficiencyService.delete(proficiencyId).subscribe((e) => {
