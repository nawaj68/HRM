...
@Component({
  selector: 'app-opening-year-list',
  templateUrl: './opening-year-list.component.html',
  styleUrls: ['./opening-year-list.component.scss']
})
export class OpeningYearListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<OpeningYear> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public openingYearService: OpeningYearService, public routeService: RouteService
    ...
    load(): any {
      return this.openingYearService.getSearch(
    ...
    remove(openingYearId: number): void {
      if (openingYearId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.openingYearService.delete(openingYearId).subscribe((e) => {
