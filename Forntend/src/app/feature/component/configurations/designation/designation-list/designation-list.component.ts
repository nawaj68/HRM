...
export class DesignationListComponent implements OnInit {
  ...
  customSubscribe = (data: any) => {
    this.designation = data.data.data;
  ...
  load(): any {
    return this.designationService.getSearch(
  ...
  remove(designationId: number): void {
    if (designationId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.designationService.delete(designationId).subscribe((e) => {
