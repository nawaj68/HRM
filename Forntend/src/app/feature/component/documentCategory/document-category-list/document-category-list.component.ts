...
export class DocumentCategoryListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<DocumentCategory> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public documentCategoryService: DocumentcategoryService, public routeService: RouteService
  ...
  remove(documentCategoryId: number): void {
    if (documentCategoryId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.documentCategoryService.delete(documentCategoryId).subscribe((e) => {
