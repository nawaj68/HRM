...
@Component({
  selector: 'app-functionaldesignation-list',
  templateUrl: './functionaldesignation-list.component.html',
  styleUrls: ['./functionaldesignation-list.component.scss']
})
export class FunctionaldesignationListComponent implements OnInit {
  ...
  users: FunctionalDesignation[];

  displayedColumns: string[] = ["employeeId","remark","action"];
  dataSource: MatTableDataSource<FunctionalDesignation> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public functionalDesignationService: FunctionaldesignationService, public routeService: RouteService
  ...
  load(): any {
    return this.functionalDesignationService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.functionalDesignationService.delete(id).subscribe((e) => {
