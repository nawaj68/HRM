...
export class DocumentListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["avatar","employeeId","remark","action"];
  ...
  constructor(public dialog: MatDialog, public documentService: DocumentService, public routeService: RouteService
  ...
  load(): any {
    return this.documentService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.documentService.delete(id).subscribe((e) => {
