...
@Component({
  selector: 'app-employee-management-category-list',
  templateUrl: './employee-management-category-list.component.html',
  styleUrls: ['./employee-management-category-list.component.scss']
})
export class EmployeeManagementCategoryListComponent implements OnInit {

  frontRoute = FrontRoute;
  employeesId: number;
  employeeManagementCategory: EmployeeManagementCategory;
  employeeManagementCategorys: EmployeeManagementCategory[];

  displayedColumns: string[] = ["employeeId","effectedDate","remark","action"];
  dataSource: MatTableDataSource<EmployeeManagementCategory> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public employeeManagementCategoryService: EmployeeManagementCategoryService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.employeeManagementCategorys = data.data.data;
  ...
  load(): any {
    return this.employeeManagementCategoryService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.employeeManagementCategoryService.delete(id).subscribe((e) => {
