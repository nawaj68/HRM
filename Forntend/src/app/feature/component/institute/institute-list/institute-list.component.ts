...
export class InstituteListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["companyInfo","instituteName","address","action"];
  dataSource: MatTableDataSource<Institute> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public instituteService: InstituteService, public routeService: RouteService
  ...
  load(): any {
    return this.instituteService.getSearch(
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.instituteService.delete(projectId).subscribe((e) => {
