...
export class WarningListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<Warning> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public warningService: WarningService, public routeService: RouteService
  ...
  load(): any {
    return this.warningService.getSearch(
  ...
  remove(warningId: number): void {
    if (warningId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.warningService.delete(warningId).subscribe((e) => {
