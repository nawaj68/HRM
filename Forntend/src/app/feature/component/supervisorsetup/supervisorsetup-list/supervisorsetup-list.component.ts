...
@Component({
  selector: 'app-supervisorsetup-list',
  templateUrl: './supervisorsetup-list.component.html',
  styleUrls: ['./supervisorsetup-list.component.scss']
})
export class SupervisorsetupListComponent implements OnInit {

  frontRoute = FrontRoute;
  employeesId: number;
  companyId:number;
  supervisorsetup: SupervisorSetup;
  supervisorsetups: SupervisorSetup[];

  displayedColumns: string[] = ["employeeId","companyId","action"];
  dataSource: MatTableDataSource<SupervisorSetup> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public supervisorsetupService: SupervisorsetupService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.supervisorsetups = data.data.data;
  ...
  load(): any {
    return this.supervisorsetupService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.supervisorsetupService.delete(id).subscribe((e) => {
