...
@Component({
  selector: 'app-job-base-status-list',
  templateUrl: './job-base-status-list.component.html',
  styleUrls: ['./job-base-status-list.component.scss']
})
export class JobBaseStatusListComponent implements OnInit {
  frontRoute = FrontRoute;
  userId: number;
  jobbasestatusId: number;
  jobbasestatus: JobBaseStatus;
  jobbasestatuss: JobBaseStatus[];

  displayedColumns: string[] = ["companyInfo","jobBaseStatusCode","jobBaseStatusTitle","project","action"];
  dataSource: MatTableDataSource<JobBaseStatus> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public jobbasestatusService: JobBaseStatusService, public routeService: RouteService
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.jobbasestatusService.delete(projectId).subscribe((e) => {
