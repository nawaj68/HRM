...
@Component({
  selector: 'app-newjobstatus-list',
  templateUrl: './newjobstatus-list.component.html',
  styleUrls: ['./newjobstatus-list.component.scss']
})
export class NewjobstatusListComponent implements OnInit {
  frontRoute = FrontRoute;
  userId: number;
  newjobstatusId: number;
  newjobstatus: Newjobstatus;
  newjobstatuss: Newjobstatus[];

  displayedColumns: string[] = ["companyInfo","statusCode","statusTitle","project","action"];
  dataSource: MatTableDataSource<Newjobstatus> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public newjobstatusService: NewjobstatusService, public routeService: RouteService
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.newjobstatusService.delete(projectId).subscribe((e) => {
