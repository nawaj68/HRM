...
@Component({
  selector: 'app-distribute-asset-list',
  templateUrl: './distribute-asset-list.component.html',
  styleUrls: ['./distribute-asset-list.component.scss']
})
export class DistributeAssetListComponent implements OnInit {
  frontRoute = FrontRoute;

  userId: number;
  distributeAssetId: number;
  user: DistributeAsset;
  users: DistributeAsset[];

  displayedColumns: string[] = ["employeeId","assetId", "remarks","action"];
  dataSource: MatTableDataSource<DistributeAsset> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public distributeAssetService: DistributeAssetService, public routeService: RouteService
    ...
    load(): any {
      return this.distributeAssetService.getSearch(
    ...
    remove(distributeAssetId: number): void {
      if (distributeAssetId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.distributeAssetService.delete(distributeAssetId).subscribe((e) => {
