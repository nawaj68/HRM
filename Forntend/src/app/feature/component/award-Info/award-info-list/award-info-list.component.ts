...
@Component({
  selector: 'app-award-info-list',
  templateUrl: './award-info-list.component.html',
  styleUrls: ['./award-info-list.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class AwardInfoListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["avatar", "awardName", "gift", "priceAmount", "remark", "action"];
  dataSource: MatTableDataSource<AwardInfo> = new MatTableDataSource();
  ...
  customSubscribe = (data: any) => {
    this.awardInfos = data.data.data;
  ...
  load(): any {
    return this.awardInfoService.getSearch(
  ...
  remove(awardInfoId: number): void {
    if (awardInfoId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.awardInfoService.delete(awardInfoId).subscribe((e) => {
