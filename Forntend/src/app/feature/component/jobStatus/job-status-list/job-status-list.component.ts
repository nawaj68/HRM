...
@Component({
  ...
  styleUrls: ['./job-status-list.component.scss']
})
export class JobStatusListComponent implements OnInit {
  ...
  jobstatus: JobStatus[];
  jobstatuss: JobStatus[];

  displayedColumns: string[] = ["employeeId", "statusId", "effectedDate", "remark", "action"];
  dataSource: MatTableDataSource<JobStatus> = new MatTableDataSource();
  ...
  load(): any {
    return this.jobstatusService.getSearch(
  ...
  remove(jobstatusId: number): void {
    if (jobstatusId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.jobstatusService.delete(jobstatusId).subscribe((e) => {
