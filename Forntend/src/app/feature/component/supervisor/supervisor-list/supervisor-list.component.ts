...
export class SupervisorListComponent implements OnInit {
  ...
  userId: number;
  employeesId: number;
  suppervisorSetupId:number;
  user: Supervisor;
  users: Supervisor[];

  displayedColumns: string[] = ["employeeId","suppervisorSetupId","remark","action"];
  dataSource: MatTableDataSource<Supervisor> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public supervisorService: SupervisorService, public routeService: RouteService
  ...
  load(): any {
    return this.supervisorService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.supervisorService.delete(id).subscribe((e) => {
