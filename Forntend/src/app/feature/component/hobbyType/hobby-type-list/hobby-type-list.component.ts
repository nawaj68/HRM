...
@Component({
  selector: 'app-hobby-type-list',
  templateUrl: './hobby-type-list.component.html',
  styleUrls: ['./hobby-type-list.component.scss']
})
export class HobbyTypeListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<HobbyType> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public hobbyTypeService: HobbyTypeService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.hobbyTypes = data.data.data;
  ...
  load(): any {
    return this.hobbyTypeService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.hobbyTypeService.delete(id).subscribe((e) => {
