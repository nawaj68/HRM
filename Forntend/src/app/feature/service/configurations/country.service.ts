...
export class CountryService extends HttpService<Country> {
  ...
  updateCountryDetail<T>(countryId: number, branchInfo  : any): Observable<T> {
    return this.http.put<T>(`${this.BaseUrl}/${countryId}`, branchInfo).pipe(catchError(this.handleError));
  }
}
