...
@Component({
  selector: 'app-branch-info-list',
  templateUrl: './branch-info-list.component.html',
  styleUrls: ['./branch-info-list.component.scss'],
...
export class BranchInfoListComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public branchInfoService: BranchInfoService, public routeService: RouteService
  ...
  load(): any {
    return this.branchInfoService.getSearch(
  ...
  remove(branchInfoId: number): void {
    if (branchInfoId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.branchInfoService.delete(branchInfoId).subscribe((e) => {
