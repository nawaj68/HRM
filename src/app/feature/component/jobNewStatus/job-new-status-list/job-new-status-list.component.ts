...
@Component({
  selector: 'app-job-new-status-list',
  templateUrl: './job-new-status-list.component.html',
  styleUrls: ['./job-new-status-list.component.scss']
})
export class JobNewStatusListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<JobNewStatus> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public jobNewStatusService: JobNewStatusService, public routeService: RouteService
    ...
    load(): any {
      return this.jobNewStatusService.getSearch(
    ...
    remove(jobNewStatusId: number): void {
      if (jobNewStatusId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.jobNewStatusService.delete(jobNewStatusId).subscribe((e) => {
