...
export class ClaimTypeListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<ClaimType> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public claimTypeService: CliamTypeService, public routeService: RouteService
    ...
    load(): any {
      return this.claimTypeService.getSearch(
    ...
    remove(claimTypeId: number): void {
      if (claimTypeId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.claimTypeService.delete(claimTypeId).subscribe((e) => {
