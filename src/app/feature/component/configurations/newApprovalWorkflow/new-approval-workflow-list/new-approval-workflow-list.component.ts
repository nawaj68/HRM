...
@Component({
  selector: 'app-new-approval-workflow-list',
  templateUrl: './new-approval-workflow-list.component.html',
  styleUrls: ['./new-approval-workflow-list.component.scss']
})
export class NewApprovalWorkflowListComponent implements OnInit {

  frontRoute = FrontRoute;

  userId: number;
  newApprovalWorkflowId: number;
  user: NewApprovalWorkflow;
  users: NewApprovalWorkflow[];

  displayedColumns: string[] = ["companyId","newApprovalWorkflowName", "remarks","action"];
  dataSource: MatTableDataSource<NewApprovalWorkflow> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public newApprovalWorkflowService: NewApprovalWorkflowService, public routeService: RouteService
    ...
    load(): any {
      return this.newApprovalWorkflowService.getSearch(
    ...
    remove(newApprovalWorkflowId: number): void {
      if (newApprovalWorkflowId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.newApprovalWorkflowService.delete(newApprovalWorkflowId).subscribe((e) => {
