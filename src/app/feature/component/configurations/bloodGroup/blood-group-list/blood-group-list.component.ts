...
export class BloodGroupListComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public bloodGroupService: BloodgroupService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.bloodGroups = data.data.data;
  ...
  load(): any {
    return this.bloodGroupService.getSearch(
  ...
  remove(bloodgroupId: number): void {
    if (bloodgroupId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.bloodGroupService.delete(bloodgroupId).subscribe((e) => {
