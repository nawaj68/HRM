...
export class UserInformationListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["avatar","firstname", "lastname", "mobileNumber", "email", "gender", "maritalStatus", "action"];
  ...
  constructor(public dialog: MatDialog, public userService: UserInformationService, public routeService: RouteService
  ...
  remove(userinformationId: number): void {
    if (userinformationId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.userService.delete(userinformationId).subscribe((e) => {
