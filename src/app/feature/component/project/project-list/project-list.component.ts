...
export class ProjectListComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public projectService: ProjectService, public routeService: RouteService
  ...
  load(): any {
    return this.projectService.getSearch(
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.projectService.delete(projectId).subscribe((e) => {
