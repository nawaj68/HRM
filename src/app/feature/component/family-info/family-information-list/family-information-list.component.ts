...
@Component({
  selector: 'app-family-information-list',
  templateUrl: './family-information-list.component.html',
  styleUrls: ['./family-information-list.component.scss']
})
export class FamilyInformationListComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public userService: FamilyInfoService, public routeService: RouteService
  ...
  remove(familyInfoId: number): void {
    if (familyInfoId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.userService.delete(familyInfoId).subscribe((e) => {
