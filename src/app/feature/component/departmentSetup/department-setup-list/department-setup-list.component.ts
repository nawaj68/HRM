...
@Component({
  selector: 'app-department-setup-list',
  templateUrl: './department-setup-list.component.html',
  styleUrls: ['./department-setup-list.component.scss']
})
export class DepartmentSetupListComponent implements OnInit {
  ...
  departmentSetups: DepartmentSetup[];

  displayedColumns: string[] = ["employeeId", "departmentId", "effectedDate", "remark", "action"];
  dataSource: MatTableDataSource<DepartmentSetup> = new MatTableDataSource();
  ...
  customSubscribe = (data: any) => {
    this.departmentSetup = data.data.data;
  ...
  load(): any {
    return this.departmentSetupService.getSearch(
  ...
  remove(departmentSetupId: number): void {
    if (departmentSetupId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.departmentSetupService.delete(departmentSetupId).subscribe((e) => {
