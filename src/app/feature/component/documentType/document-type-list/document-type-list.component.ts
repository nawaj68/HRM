...
export class DocumentTypeListComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public documentTypeService: DocumentTypeService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.documentTypes = data.data.data;
  ...
  load(): any {
    return this.documentTypeService.getSearch(
  ...
  remove(id: number): void {
    if (id) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.documentTypeService.delete(id).subscribe((e) => {
