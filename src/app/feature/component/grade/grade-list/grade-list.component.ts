...
export class GradeListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["companyInfo","gradeName","gradePoint","action"];
  ...
  constructor(public dialog: MatDialog, public gradeService: GradeService, public routeService: RouteService
  ...
  load(): any {
    return this.gradeService.getSearch(
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.gradeService.delete(projectId).subscribe((e) => {
