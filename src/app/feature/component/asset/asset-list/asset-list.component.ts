...
export class AssetListComponent implements OnInit {
  ...
  displayedColumns: string[] = ["assetName","assetCode","companyInfo","serialNumber","modelNumber","action"];
  ...
  constructor(public dialog: MatDialog, public assetService: AssetService, public routeService: RouteService
  ...
  load(): any {
    return this.assetService.getSearch(
  ...
  remove(projectId: number): void {
    if (projectId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.assetService.delete(projectId).subscribe((e) => {
