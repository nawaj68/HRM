...
@Component({
  ...
  styleUrls: ['./companyinfo-list.component.scss']
})
export class CompanyinfoListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<CompanyInfo> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public companyinfoService: CompanyinfoService, public routeService: RouteService
  ...
  load(): any {
    return this.companyinfoService.getSearch(
  ...
  remove(companyinfoId: number): void {
    if (companyinfoId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.companyinfoService.delete(companyinfoId).subscribe((e) => {
