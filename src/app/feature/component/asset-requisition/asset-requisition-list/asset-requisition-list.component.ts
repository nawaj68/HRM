...
@Component({
  selector: 'app-asset-requisition-list',
  templateUrl: './asset-requisition-list.component.html',
  styleUrls: ['./asset-requisition-list.component.scss']
})
export class AssetRequisitionListComponent implements OnInit {
  frontRoute = FrontRoute;

  userId: number;
  assetRequisitionId: number;
  user: AssetRequisition;
  users: AssetRequisition[];

  displayedColumns: string[] = ["employeeId","assetId", "approvalDate", "remarks","action"];
  dataSource: MatTableDataSource<AssetRequisition> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public assetRequisitionService: AssetRequisitionService, public routeService: RouteService
    ...
    load(): any {
      return this.assetRequisitionService.getSearch(
    ...
    remove(assetRequisitionId: number): void {
      if (assetRequisitionId) {
        this.messageService.confirm().then((result) => {
          if (result.value) {
            this.assetRequisitionService.delete(assetRequisitionId).subscribe((e) => {
