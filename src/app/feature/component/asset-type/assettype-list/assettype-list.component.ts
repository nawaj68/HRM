...
export class AssettypeListComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<AssetType> = new MatTableDataSource();
  ...
  constructor(public dialog: MatDialog, public assettypeService: AssettypeService, public routeService: RouteService
  ...
  customSubscribe = (data: any) => {
    this.assettypes = data.data.data;
  ...
  load(): any {
    return this.assettypeService.getSearch(
  ...
  remove(employeesId: number): void {
    if (employeesId) {
      this.messageService.confirm().then((result) => {
        if (result.value) {
          this.assettypeService.delete(employeesId).subscribe((e) => {
